   ...
   <div class="row page-titles mx-0">
      <div class="col p-md-0">
         <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="javascript:void(0)">View Shift</a></li>
         </ol>
      </div>
   </div>
   <!-- row -->
            ...
            <div class="card">
               <div class="card-body">
                  <div class="card-title">
                     <button class="btn btn-primary btn-rounded" data-toggle="modal" data-target="#AddStudentShift" style="float: right;">Add new</button>
                  </div>
                  @if(count($allData)>=1)
                              ...
                              <table class="table table-hover" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                 ...
                                 <tbody>
                                    ...
                                    <tr role="row" class="odd">
                                       <td width='5%'>{{$key+1}} </td>
                                       <td width='20%'>{{ $shift->name}}</td>
                                       <td width='20%'>{{ $shift->created_at->diffForHumans()}} </td>
                                       <td width='15%'>
                                          <a href="#" data-toggle="modal" data-target="#ModalUpdateShift{{$shift->id}}">
                                             <i class="fa fa-pencil color-muted m-r-5"></i>
                                          </a> &nbsp;
                                          <a href="{{ route('student.shift.delete',$shift->id)}}" id="delete">
                                             <i class="fa fa-trash color-muted m-r-5"></i>
                                          </a>
                                          <!--Student Shift update-->
                                          <form action="{{ route('student.shift.update',$shift->id)}}" method="post" autocomplete="off">
                                             @csrf
                                             <div class="modal fade" id="ModalUpdateShift{{$shift->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                                <div class="modal-dialog">
                                                   <div class="modal-content">
                                                      <div class="modal-header">
                                                         <h5 class="modal-title">Update Student Shift</h5>
                                                      ...
                                                      <div class="modal-body">
                                                         <div class="form-group row">
                                                            ...
                                                            <div class="col-lg-7">
                                                               <input type="text" class="form-control input-rounded" id="name" name="name" value="{{ $shift->name}}">
                                                               @error('name')
                  ...
                  @else
                  <p>No data selected for student Group add new one</p>
                  @endif
               </div>
            </div>
         </div>
...
<form action="{{ route('student.shift.store')}}" method="post" autocomplete="off">
   @csrf
   <div class="modal fade bd-example-modal-lg" id="AddStudentShift" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog">
         <div class="modal-content">
            <div class="modal-header">
               <h5 class="modal-title">Add Student Shift</h5>
               <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
               </button>
            </div>
            <div class="modal-body">
               <div class="form-group row">
                  ...
                  <div class="col-lg-7">
                     <input type="text" class="form-control input-rounded" id="name" name="name" placeholder="Enter sudent shift">
                     @error('name')
                     <div class="text-danger">{{ $message }}</div>
                     @enderror
                  </div>
